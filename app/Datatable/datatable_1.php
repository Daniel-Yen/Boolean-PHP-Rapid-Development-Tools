<?php return array (
  'id' => 1,
  'title' => '页面设计',
  'id_prefix' => 'design_',
  'pid' => 46,
  'model' => '2',
  'function_type' => '2',
  'url' => 'lazykit/function_page/design',
  'method' => 'FunctionPageController@design',
  'inheritance' => NULL,
  'inheritance_note' => NULL,
  'main_table' => 'blk_function_page',
  'associated_type' => 'INNER JOIN',
  'associated_table' => 'blk_attachment',
  'external_field' => '444',
  'datatable_set' => 
  array (
    444 => 
    array (
      'field_type' => NULL,
      'field_length' => NULL,
      'sorting' => NULL,
      'fixed' => '0',
      'title' => NULL,
      'field' => '444',
      'field_from' => 'external_field',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'father_id' => 
    array (
      'field_type' => 'mediumint',
      'field_length' => '8 unsigned',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '上级id',
      'field' => 'father_id',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'sorting' => 
    array (
      'field_type' => 'int',
      'field_length' => '5',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '排序',
      'field' => 'sorting',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'main_table' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '30',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '主表',
      'field' => 'main_table',
      'field_from' => 'main_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'associated_table' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '30',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '关联表',
      'field' => 'associated_table',
      'field_from' => 'main_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'associated_type' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '15',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '关联类型',
      'field' => 'associated_type',
      'field_from' => 'main_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'external_field' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '250',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '自定义字段',
      'field' => 'external_field',
      'field_from' => 'main_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'status' => 
    array (
      'field_type' => 'tinyint',
      'field_length' => '1 unsigned',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '状态，0为无用，1为正常使用',
      'field' => 'status',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'uid' => 
    array (
      'field_type' => 'mediumint',
      'field_length' => '8 unsigned',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '用户id',
      'field' => 'uid',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'inheritance_note' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '250',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '继承备注',
      'field' => 'inheritance_note',
      'field_from' => 'main_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'name' => 
    array (
      'field_type' => 'char',
      'field_length' => '50',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '文件名',
      'field' => 'name',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'module' => 
    array (
      'field_type' => 'char',
      'field_length' => '15',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '模块名，由哪个模块上传的',
      'field' => 'module',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'path' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '255',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '文件路径',
      'field' => 'path',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'thumb' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '255',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '缩略图路径',
      'field' => 'thumb',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'mime' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '64',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '文件mime类型',
      'field' => 'mime',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'ext' => 
    array (
      'field_type' => 'char',
      'field_length' => '4',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '文件类型',
      'field' => 'ext',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'size' => 
    array (
      'field_type' => 'int',
      'field_length' => '11 unsigned',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '文件大小',
      'field' => 'size',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'md5' => 
    array (
      'field_type' => 'char',
      'field_length' => '32',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => '文件md5',
      'field' => 'md5',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'sha1' => 
    array (
      'field_type' => 'char',
      'field_length' => '40',
      'sorting' => NULL,
      'fixed' => '0',
      'title' => 'sha1 散列值',
      'field' => 'sha1',
      'field_from' => 'associated_table',
      'width' => NULL,
      'search' => '0',
      'attribute' => NULL,
    ),
    'id' => 
    array (
      'field_type' => 'int',
      'field_length' => '11',
      'sorting' => '1',
      'fixed' => '0',
      'title' => 'ID',
      'field' => 'id',
      'field_from' => 'main_table',
      'width' => '80',
      'read' => 'on',
      'search' => '0',
      'attribute' => NULL,
    ),
    'title' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '40',
      'sorting' => '2',
      'fixed' => 'left',
      'title' => '页面名称',
      'field' => 'title',
      'field_from' => 'main_table',
      'width' => '220',
      'create' => 'on',
      'update' => 'on',
      'read' => 'on',
      'search' => 'like',
      'attribute' => 
      array (
        'align' => 'left',
        'data_input_form' => 'input',
        'data_source_type' => 'method',
        'data_source' => NULL,
        'validate' => NULL,
        'dic_static' => NULL,
        'dic_table' => NULL,
        'dic_key' => NULL,
        'dic_value' => NULL,
        'function' => NULL,
        'window_title' => 'on',
        'url_type' => 'window',
        'url' => NULL,
        'cell_style_template' => NULL,
      ),
    ),
    'id_prefix' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '40',
      'sorting' => '2',
      'fixed' => '0',
      'title' => 'ID前缀',
      'field' => 'id_prefix',
      'field_from' => 'main_table',
      'width' => NULL,
      'create' => 'on',
      'update' => 'on',
      'read' => 'on',
      'search' => '0',
      'attribute' => NULL,
    ),
    'pid' => 
    array (
      'field_type' => 'int',
      'field_length' => '11',
      'sorting' => '3',
      'fixed' => '0',
      'title' => '父ID',
      'field' => 'pid',
      'field_from' => 'main_table',
      'width' => NULL,
      'create' => 'on',
      'update' => 'on',
      'search' => '=',
      'attribute' => 
      array (
        'align' => 'left',
        'data_input_form' => 'tree_select',
        'data_source_type' => 'method',
        'data_source' => 'attributePid',
        'validate' => 'required',
        'dic_static' => NULL,
        'dic_table' => NULL,
        'dic_key' => NULL,
        'dic_value' => NULL,
        'function' => NULL,
        'url_type' => 'window',
        'url' => NULL,
        'cell_style_template' => NULL,
      ),
    ),
    'model' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '20',
      'sorting' => '4',
      'fixed' => '0',
      'title' => '页面模型',
      'field' => 'model',
      'field_from' => 'main_table',
      'width' => '130',
      'create' => 'on',
      'update' => 'on',
      'read' => 'on',
      'search' => '0',
      'attribute' => 
      array (
        'align' => 'left',
        'data_input_form' => 'select',
        'data_source_type' => 'method',
        'data_source' => 'attributeModel',
        'validate' => 'required',
        'dic_static' => NULL,
        'dic_table' => NULL,
        'dic_key' => NULL,
        'dic_value' => NULL,
        'function' => NULL,
        'url_type' => 'window',
        'url' => NULL,
        'cell_style_template' => NULL,
      ),
    ),
    'inheritance' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '200',
      'sorting' => '5',
      'fixed' => '0',
      'title' => '继承自',
      'field' => 'inheritance',
      'field_from' => 'main_table',
      'width' => NULL,
      'read' => 'on',
      'search' => '0',
      'attribute' => 
      array (
        'align' => 'left',
        'data_input_form' => 'select',
        'data_source_type' => 'method',
        'data_source' => 'attributeInheritance',
        'validate' => NULL,
        'dic_static' => NULL,
        'dic_table' => NULL,
        'dic_key' => NULL,
        'dic_value' => NULL,
        'function' => NULL,
        'url_type' => 'window',
        'url' => NULL,
        'cell_style_template' => NULL,
      ),
    ),
    'function_type' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '20',
      'sorting' => '6',
      'fixed' => '0',
      'title' => '页面类型',
      'field' => 'function_type',
      'field_from' => 'main_table',
      'width' => '90',
      'create' => 'on',
      'update' => 'on',
      'read' => 'on',
      'search' => '0',
      'attribute' => 
      array (
        'align' => 'left',
        'data_input_form' => 'select',
        'data_source_type' => 'method',
        'data_source' => 'attributeFunctionType',
        'validate' => 'required',
        'dic_static' => NULL,
        'dic_table' => NULL,
        'dic_key' => NULL,
        'dic_value' => NULL,
        'function' => NULL,
        'url_type' => 'window',
        'url' => NULL,
        'cell_style_template' => NULL,
      ),
    ),
    'url' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '50',
      'sorting' => '7',
      'fixed' => '0',
      'title' => '路由',
      'field' => 'url',
      'field_from' => 'main_table',
      'width' => '200',
      'read' => 'on',
      'search' => '0',
      'attribute' => NULL,
    ),
    'method' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '200',
      'sorting' => '8',
      'fixed' => '0',
      'title' => '处理方法',
      'field' => 'method',
      'field_from' => 'main_table',
      'width' => '250',
      'create' => 'on',
      'update' => 'on',
      'read' => 'on',
      'search' => '0',
      'attribute' => NULL,
    ),
    'system_id' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '20',
      'sorting' => '9',
      'fixed' => '0',
      'title' => '所属系统',
      'field' => 'system_id',
      'field_from' => 'main_table',
      'width' => NULL,
      'read' => 'on',
      'search' => '0',
      'attribute' => 
      array (
        'align' => 'left',
        'data_input_form' => 'select',
        'data_source_type' => 'method',
        'data_source' => 'attributeSystem',
        'validate' => 'required',
        'dic_static' => NULL,
        'dic_table' => NULL,
        'dic_key' => NULL,
        'dic_value' => NULL,
        'function' => NULL,
        'url_type' => 'window',
        'url' => NULL,
        'cell_style_template' => NULL,
      ),
    ),
    'module_id' => 
    array (
      'field_type' => 'varchar',
      'field_length' => '20',
      'sorting' => '10',
      'fixed' => '0',
      'title' => '所属模块',
      'field' => 'module_id',
      'field_from' => 'main_table',
      'width' => NULL,
      'create' => 'on',
      'update' => 'on',
      'read' => 'on',
      'search' => '0',
      'attribute' => 
      array (
        'align' => 'left',
        'data_input_form' => 'select',
        'data_source_type' => 'method',
        'data_source' => 'attributeModule',
        'validate' => 'required',
        'dic_static' => NULL,
        'dic_table' => NULL,
        'dic_key' => NULL,
        'dic_value' => NULL,
        'function' => NULL,
        'url_type' => 'window',
        'url' => NULL,
        'cell_style_template' => NULL,
      ),
    ),
    'created_at' => 
    array (
      'field_type' => 'datetime',
      'field_length' => 'no_limit',
      'sorting' => '999',
      'fixed' => '0',
      'title' => '创建时间',
      'field' => 'created_at',
      'field_from' => 'main_table',
      'width' => NULL,
      'read' => 'on',
      'search' => '0',
      'attribute' => NULL,
    ),
    'updated_at' => 
    array (
      'field_type' => 'datetime',
      'field_length' => 'no_limit',
      'sorting' => '999',
      'fixed' => '0',
      'title' => '修改时间',
      'field' => 'updated_at',
      'field_from' => 'main_table',
      'width' => NULL,
      'read' => 'on',
      'search' => '0',
      'attribute' => NULL,
    ),
    'deleted_at' => 
    array (
      'field_type' => 'datetime',
      'field_length' => 'no_limit',
      'sorting' => '999',
      'fixed' => '0',
      'title' => '删除时间',
      'field' => 'deleted_at',
      'field_from' => 'associated_table',
      'width' => NULL,
      'read' => 'on',
      'search' => '0',
      'attribute' => NULL,
    ),
  ),
  'head_menu' => 
  array (
    'search' => 
    array (
      'text' => '搜索',
      'icon' => 'layui-icon-search',
      'must' => 'on',
      'width' => '800px',
      'height' => '90%',
      'method' => NULL,
    ),
    'create' => 
    array (
      'text' => '新增',
      'icon' => 'layui-icon-add-1',
      'must' => 'on',
      'width' => '100%',
      'height' => '100%',
      'method' => NULL,
    ),
    'update' => 
    array (
      'text' => '修改',
      'icon' => 'layui-icon-edit',
      'must' => 'on',
      'width' => '100%',
      'height' => '100%',
      'method' => NULL,
    ),
    'delete' => 
    array (
      'text' => '删除',
      'icon' => 'layui-icon-delete',
      'must' => 'on',
      'width' => NULL,
      'height' => NULL,
      'method' => NULL,
    ),
    'recycle' => 
    array (
      'text' => '回收站',
      'icon' => 'layui-icon-fonts-del',
      'must' => 'on',
      'width' => '100%',
      'height' => '100%',
      'method' => NULL,
    ),
    'recovery' => 
    array (
      'text' => '数据恢复',
      'icon' => 'layui-icon-prev',
      'must' => 'on',
      'width' => NULL,
      'height' => NULL,
      'method' => NULL,
    ),
  ),
  'other_set' => 
  array (
    'is_tree' => 'on',
    'limit' => '90',
    'cell_min_width' => '160',
    'line_button_area_width' => '90',
  ),
  'route' => 
  array (
    'route_path' => 'lazykit/function_page/',
    'route_name' => 'design',
    'controller' => 'App\\Http\\Controllers\\Lazykit\\FunctionPageController',
    'method' => 'design',
  ),
  'new_head_menu' => 
  array (
    'route' => 
    array (
      'text' => '更新路由',
      'icon' => NULL,
      'open_tepe' => 'ajax',
      'must' => 'on',
      'width' => NULL,
      'height' => NULL,
      'method' => 'createRoute',
    ),
    'menu' => 
    array (
      'text' => '更新菜单',
      'icon' => NULL,
      'open_tepe' => 'ajax',
      'must' => 'on',
      'width' => NULL,
      'height' => NULL,
      'method' => 'createMenu',
    ),
    'permission' => 
    array (
      'text' => '更新授权',
      'icon' => NULL,
      'open_tepe' => 'ajax',
      'must' => 'on',
      'width' => NULL,
      'height' => NULL,
      'method' => 'createPermissions',
    ),
  ),
  'line_button' => 
  array (
    'set' => 
    array (
      'text' => '更新配置',
      'style' => 'layui-btn-danger',
      'open_tepe' => 'window',
      'must' => 'on',
      'width' => '100%',
      'height' => '100%',
      'method' => 'set',
    ),
  ),
);?>