...
class ConfigController extends Controller
{
    ...
    public function createConfig($config_name, $request)
    {
    	...
    	if($request->isMethod('post')){
			...
			foreach($request->config as $k=>$v){
				ConfigRepository::updateOrInsert(
					['tag' 		=> $k],
    	...
    	//获得要配置的值
    	$config_arr = ConfigRepository::whereIn('tag', $ids)->get();
    	if($config_arr->first()){
    	...
    	$content = view('booleanTools.config.form');
	...
	{
		$config_arr = ConfigRepository::where('tag', '=', $ids)->get();
		if($config_arr->first()){
