...
class DatatableController extends Controller
{
	 ...
	 *		9、【已实现】自定义新增页面
	 *		'create_page' => 'booleanTools.datatable.form',  
	 *		10、【已实现】自定义修改页面
	 *		'update_page' => 'booleanTools.datatable.form',	
	 *	];
    ...
    public function createDatatable($config_name, $additional_config = [], $request)
    {
		...
		if( $request->do == "create") {
			...
			if($request->isMethod('post')){
				...
				if(isset($request->boolean_success)?$request->boolean_success == 'success':false){
					echo success("添加成功");
					die();
				}else{
			...
			echo view('booleanTools.datatable.import', [
			...
			if($template && 0){
				...
				$content = view('booleanTools.datatable.body');
				$template = response($content)->getContent();
				//cache($config_name, $template);
				echo $template;
			}
	...
	{
		$datatable_config = get_boolean_tools_config($config_name);
		//dd($datatable_config);
		
		if(!empty($datatable_config)){
			//处理模型中的继承关系
			if(isset($datatable_config['inheritance'])?$datatable_config['inheritance']:false){
				$inheritance_datatable_config = get_boolean_tools_config('datatable_'.$datatable_config['inheritance']);
			...
			//9、自定义新增页面
			$datatable_config['create_page'] = isset($additional_config['create_page'])?$additional_config['create_page']:'booleanTools.datatable.form';
			
			//10、自定义修改页面
			$datatable_config['update_page'] = isset($additional_config['update_page'])?$additional_config['update_page']:'booleanTools.datatable.form';
			//dd($datatable_config);
			...
			if(isset($datatable_config['new_head_menu'])){
				foreach($datatable_config['new_head_menu'] as $k=>$v){
					//根据权限判断可用按钮
					if(!in_array($k, $boolean_rules) && $k != 'search'){
